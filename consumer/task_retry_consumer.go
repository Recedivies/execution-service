...
const (
	queueTaskRetry    = "q.task-retry"
	consumerTaskRetry = "task-retry"

	delay = 10 * time.Second
)

func (processor *RabbitMQTaskProcessor) ListenTaskRetryAndServe() error {
	msgs, err := processor.rmq.Channel.Consume(
		queueTaskRetry,    // queue
		consumerTaskRetry, // consumer
		false,             // auto-ack
		false,             // exclusive
		false,             // no-local
		false,             // no-wait
		...
		for msg := range msgs {
			...
			if nack {
				...
				if taskExecutionHistory.RetryCount <= job.MaxRetryCount {
					...
					log.Info().Msg("Published to dlq task queue")
				}
				taskExecutionHistory.ExecutionTime = time.Now()
				processor.DB.Save(&taskExecutionHistory)
